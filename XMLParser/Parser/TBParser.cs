...
using System.Xml;
using XmlObjects;
using XMLParser.Converters;
using XMLParser.Data;

namespace XMLParser
                {
    public class TBParser : AbstractFactoryParser
                    {
        ...
        public override Import GetPayments(string path)
        {
            Parse(path);
        ...
        {
            _report = (OFX)Deserializer.Manager.Deserialize<OFX>(path);
        }
    }
}